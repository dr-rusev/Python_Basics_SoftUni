...
# за да не останат гладни. Напишете програма, която пресмята колко килограма храна
# ще изядат всички за времето, в което Марина отсъства и дали оставената храна от нея ще им е достатъчна.
...
# 1 gram = kg /1000
# gram/day = kg/(day*1000)
import math
days = int(input())
food_left_kg = int(input())
dog_per_day_kg = float(input())
cat_per_day_kg = float(input())
turtle_per_day_g = float(input())
consumed_food = days * (dog_per_day_kg + cat_per_day_kg + turtle_per_day_g /1000)
remain_food = food_left_kg - consumed_food
if food_left_kg >= consumed_food:
    remain_food = math.floor(remain_food)
    print(f'{remain_food}' + " kilos of food left.")
elif food_left_kg < consumed_food:
    remain_food = abs(remain_food)
    remain_food = math.ceil(remain_food)
    print(f'{remain_food}' + " more kilos of food are needed.")